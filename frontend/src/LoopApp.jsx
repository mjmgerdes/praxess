              ...
              <div style={css(`background:#fff;border:1px solid oklch(0.91 0.008 255);border-radius:12px;overflow:hidden;`)}>
                <div style={css(`padding:12px 16px;border-bottom:1px solid oklch(0.93 0.006 258);display:flex;align-items:center;justify-content:space-between;`)}>
                  ...
                  <div style={css(`display:flex;align-items:center;gap:8px;`)}>
                    ...
                    <button onClick={V.downloadAppealPdf} style={css(`padding:7px 13px;border-radius:8px;border:none;background:oklch(0.45 0.12 255);color:#fff;font-family:'IBM Plex Sans',sans-serif;font-size:12px;font-weight:600;cursor:pointer;white-space:nowrap;`)}>↓ Download letter · PDF</button>
                  </div>
                </div>
                <div style={css(`padding:18px 20px;font-family:Georgia,'Times New Roman',serif;font-size:13px;line-height:1.6;color:oklch(0.28 0.02 258);max-height:420px;overflow:auto;`)}>
                  <p style={css(`margin:0 0 12px;`)}>{new Date().toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}</p>
                  <p style={css(`margin:0 0 12px;`)}>Meridian Health Plan<br/>Prior Authorization Review Department<br/>Via eviCore (RBM)</p>
                  <p style={css(`margin:0 0 12px;font-weight:700;`)}>RE: First Level Appeal — Prior Authorization Denial<br/>Denial Reference: {V.denialRef || '[payer denial reference]'}<br/>Procedure: MRI lumbar spine without contrast (CPT 72148)<br/>Patient: Kovacek, Emory · Member MHP-88231-04 · Case PA-4471</p>
                  <p style={css(`margin:0 0 12px;`)}>Dear Prior Authorization Review Board,</p>
                  <p style={css(`margin:0 0 12px;`)}>We write to formally appeal the denial referenced above. The denial states: <em>“{V.denialReason || 'insufficient documentation of the conservative-therapy requirement'}”</em>. The documented clinical record satisfies, point by point, the medical-necessity criteria your plan applies to this service (ACR Appropriateness Criteria — Low Back Pain, 2021, administered via eviCore).</p>
                  <p style={css(`margin:0 0 12px;`)}>First: the clinician-approved note addendum documents the self-directed conservative-care course — ibuprofen with good effect until the supply ran out, alongside walking and heat — grounded in the verbatim encounter transcript <span style={css(`background:oklch(0.95 0.03 250);border-radius:3px;padding:0 3px;`)}>(“ibuprofen for a while, which worked” — verified span)</span>. Second: the enclosed Metro Physical Therapy discharge summary verifies a completed <strong>8-session physical-therapy course (Jan–Mar)</strong>, previously patient-reported and now record-verified. Third: the red-flag and neurologic screen is documented in the signed encounter note.</p>
                  <p style={css(`margin:0 0 12px;`)}>All five policy criteria are satisfied with source-linked evidence. We request that the denial be reversed and authorization issued without further delay; the full criterion-by-criterion compliance analysis accompanies this letter.</p>
                  <p style={css(`margin:0 0 4px;`)}>Sincerely,</p>
                  <p style={css(`margin:0;`)}>A. Verma, MD · Lakeview Orthopedic &amp; Spine<br/><span style={css(`font-size:11.5px;color:oklch(0.5 0.02 258);`)}>cc: Emory Kovacek, patient</span></p>
                  <div style={css(`margin-top:14px;font-family:'IBM Plex Mono',monospace;font-size:10px;color:oklch(0.6 0.015 258);border-top:1px dashed oklch(0.9 0.008 255);padding-top:10px;`)}>{V.appealPayerIntel}</div>
                </div>
