...
* **Interface (supersedes §10 layout):** the Claude-Design **7-screen loop UI** at `frontend/public/praxess-ui.html` (encounter → world model → decision engine → addendum → patient question → packet → submission/appeal). Wired live: statuses hydrate from `/api/analyze` on load; Approve round-trips `/api/decide` into the session (ENGINE LIVE chip); self-contained fallback if the API is down. All transcript copy is **verbatim** from the dataset.
